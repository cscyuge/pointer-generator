...
class KaggleDataset(data.Dataset):
    ...
    def padding(self, data, data_type="content"):
        ...
        data_pad = np.pad(data, (0, max(0, max_length - len_data)), 'constant', constant_values=(self.pad_index))[
                   :max_length]
        return data_pad
...
class ChineseDataset(data.Dataset):
    ...
    def __init__(self, folder):
        # Declare the hyperparameter
        self.srcs = pickle.load(open(os.path.join(folder, 'src_ids.pkl'), 'rb'))
        self.tars = pickle.load(open(os.path.join(folder, 'tar_ids.pkl'), 'rb'))

    def __getitem__(self, index):
        return self.srcs[index], self.tars[index]
